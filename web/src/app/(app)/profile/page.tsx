...
export default async function ProfilePage() {
  ...
  return (
    <div className="w-full pb-3">
      <div className="px-page pt-2 pb-1">
        <h1 className="text-[28px] leading-tight font-bold tracking-tight">
          Perfil
        </h1>
      </div>

      <div className="px-page flex flex-col items-center pt-4 pb-6">
        <div className="border-border bg-card text-accent-foreground shadow-app-1 flex size-[84px] items-center justify-center rounded-full border text-[28px] font-bold tracking-tight">
          {initials}
        </div>
        <p className="mt-3 text-lg font-bold">{displayName}</p>
        <p className="text-muted-foreground mt-1 text-[12.5px] font-medium">
          {user.email}
        </p>
      </div>
        ...
        {goal ? (
          <div className="px-4 py-3.5">
            <div className="mb-3 flex items-center justify-between">
              {goal.is_auto && (
                <span className="bg-secondary text-accent-foreground inline-flex h-6 items-center gap-1.5 rounded-full px-2 text-[11px] font-semibold">
              ...
              <Link
                href="/profile/edit"
                className="text-accent-foreground hover:text-foreground ml-auto text-[12.5px] font-semibold"
              >
                Personalizar
              </Link>
            </div>
...
function ProfileSection({
  ...
  return (
    <section className="px-page mb-3.5">
  ...
  return (
    <div
      className={`flex items-center justify-between px-4 py-3.5 ${
        last ? "" : "border-border/80 border-b"
      }`}
    >
...
function GoalCell({
  ...
  return (
    <div className="bg-secondary rounded-[10px] px-1.5 py-2.5 text-center">
      ...
      <p className="text-muted-foreground mt-0.5 text-[9.5px]">{unit}</p>
    </div>
  );
}
